...
public class CandidateSolution implements Cloneable {
	...
	public void FillCandAssignHashTable() {
		// populating the Hashtable with the CandidateAssignment instances
		// respective
	...
	public void PrintContents() {
		...
		Enumeration<CandidateAssignment> eStudentEntry = candidateAssignements.elements(); // getting
																							// elements
																							// in
																							// the
																							// hashtable
		...
		while (eStudentEntry.hasMoreElements()) {
			vStudentDetails.add((CandidateAssignment) eStudentEntry.nextElement()); // adding
																					// the
																					// details
																					// to
																					// the
		...
		for (int i = 0; i < count; i++) {
			...
			StudentEntry std = this.preferenceTable.getEntryFor(StudentName);
			...
			for (int toPrepend = 130
					- (StudentName + "   " + space + AssignedProject).length(); toPrepend > 0; toPrepend--) {
				space1.append('*');
			}
			////////////////////////
			switch (rank) {
			...
			///////////////////////
			System.out.println(StudentName + "   " + space + AssignedProject + space1 + rank);
		}
		for (int k = 0; k < 10; k++) {
			System.out.println("Pref " + k + ": " + prefRanks[k]);
		}
	...
	public CandidateAssignment getRandomAssignment() {
		// returning an instance of CandidateAssignment which is chosen at
		// random
	...
	public int getFitness() {
		// rather than returning the inverse of the Energy, return the negative
		// of Energy
	...
	// Enumeration<CandidateAssignment> canAssignEnum =
	// candidateAssignements.elements();
	// // Looping through all the CandidateAssignments and calculating the
	// energy for each and
	...
	// Enumeration<CandidateAssignment> canAssignEnum =
	// candidateAssignements.elements();
	// // Looping through each CandidateAssignments, add the assigned project in
	// CandidateAssignments
	// // to a string variable, check whether the added CandidateAssignments
	// project exists in the Hashtable.
	// // If exists, add a penalty of 1000, else, add the CandidateAssignments
	// project to the
